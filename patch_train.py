...
def train(args):
    ...
    # Setup Augmentations
    if args.aug:
        data_aug = Compose([RandomRotate(15), RandomHorizontallyFlip(), AddNoise()])
    else:
        data_aug = None 
    ...
    if args.class_weights:
        ...
        class_weights = torch.tensor([0, 0.7151, 0.8811, 0.5156, 0.9346, 0.9683, 0.9852], device=device, requires_grad=False)
    else:
        class_weights = None
    ...
    for epoch in range(args.n_epoch):
        ...
        for i, (images, labels) in enumerate(trainloader):
            ...
            loss = loss_fn(input=outputs, target=labels ,alpha=class_weights,gamma=args.gamma, loss_type=args.loss_parameters)
            ...
            numbers = [0,14,29,49,63]
            if i in numbers:
                # number 0 image in the batch
                tb_original_image = vutils.make_grid(image_original[i][0], normalize=True, scale_each=True)
                writer.add_image('train/original_image',tb_original_image, epoch +1)
            
                labels_original=labels_original.numpy()[i]
        ...
        if args.per_val != 0:
            with torch.no_grad():  # operations inside don't track history
                ...
                for i_val, (images_val, labels_val) in tqdm(enumerate(valloader)):
                    ...
                    loss = loss_fn(input=outputs_val, target=labels_val, gamma=args.gamma,loss_type=args.loss_parameters)
                    ...
                    if i_val in numbers:
                        # number 0 image in the batch
                        tb_original_image = vutils.make_grid(image_original[i_val][0], normalize=True, scale_each=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--aug', nargs='?', type=bool, default=None,
    ...
    parser.add_argument('--loss_parameters', nargs='?', type=str, default=None,
                        help='Loss function parameters: for fully supervised model select  "baseline","LinConf_NonSim","NonConf_LinSim","LinConf_LinSim", "SoftConf_SoftSim"')
